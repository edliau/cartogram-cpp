...
// and returns the midpoint of the longest line segment that is inside the
// polygon and is halfway through the northern and southern tip of the polygon
// reference: https://gis.stackexchange.com/questions/76498/how-is-st-pointonsurface-calculated
XYPoint point_on_surface(Polygon_with_holes pwh) {

          // Calculate line_y
          CGAL::Bbox_2 bb = pwh.bbox();
          double line_y = (bb.ymin() + bb.ymax()) / 2;

          // Epsilon based on default resolution (16)
          ...
          // Assigning length of line segments (to find longest)
          // using the target_density property of intersections
          // for line segment lengths
          for (unsigned int l = 0; l < intersections.size(); l += 2) {
            intersections[l].target_density = intersections[l + 1].x -
                                             intersections[l].x;
          }

          // Finding greatest length
          double greatest_length = intersections[0].target_density;
          XYPoint left(intersections[0].x, line_y);
          XYPoint right(intersections[1].x, line_y);

          // Iterating through lengths
          for (unsigned int l = 0; l < intersections.size(); l += 2) {
            if (intersections[l].target_density > greatest_length) {
              greatest_length = intersections[l].target_density;
              left.x = intersections[l].x;
              right.x = intersections[l + 1].x;
            }
          }

          // Making final mid_point and returning it
          XYPoint mid_point;
          mid_point.x = (right.x + left.x) / 2;
          mid_point.y = line_y;
          return mid_point;
}
// example use:
    ...
    for (int j = 0; j < gd.n_polygons_with_holes(); ++j) {
      ...
      for (unsigned int k = floor(bb.ymin()) - 1;
        ...
        for (double line_y = k + (1.0/res)/2;
          ...
          // First we find all the intersections with the exterior ring
          for (unsigned int l = 0; l < ext_ring.size(); ++l) {
            XYPoint curr_point;
            curr_point.x = ext_ring[l][0];
            curr_point.y = ext_ring[l][1];
